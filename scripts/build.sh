...
GITHUB_REPOSITORY="${GITHUB_REPOSITORY:-manno/config-reloader}"
VERSION=$(git describe --tags --always --dirty 2>/dev/null || echo "dev")

export GITHUB_REPOSITORY

echo "Building config-reloader:${VERSION} with goreleaser"
...
if [[ "$VERSION" == *"dirty"* ]] || ! git describe --exact-match --tags HEAD &>/dev/null; then
    ...
    if [ -f "dist/config-reloader_linux_amd64_v1/config-reloader" ]; then
        cp dist/config-reloader_linux_amd64_v1/config-reloader ./
    elif [ -f "dist/config-reloader_linux_arm64/config-reloader" ]; then
        cp dist/config-reloader_linux_arm64/config-reloader ./
    fi
